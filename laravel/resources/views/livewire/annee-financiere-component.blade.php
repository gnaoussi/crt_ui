<div x-data x-effect="document.body.classList.toggle('overflow-hidden', $wire.isCreateModalOpen || $wire.isEditModalOpen || $wire.isDeleteModalOpen || $wire.isConfirmActionModalOpen)">
    ...
    <div class="bg-white border-b border-slate-200/80 px-6 py-2 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-xs font-semibold">
        <nav class="flex items-center space-x-2 text-slate-600">
            ...
            <span class="text-slate-600 hover:text-crt-navy cursor-pointer">Budget</span>
            <span class="text-slate-300">/</span>

            @if ($viewMode === 'detail' && $selectedAnnee)
                ...
                <span class="text-crt-navy font-extrabold bg-crt-cyan-light text-crt-navy px-2.5 py-0.5 rounded-md border border-crt-cyan/20">
                    Détails {{ $selectedAnnee['startDate'] }} - {{ $selectedAnnee['endDate'] }}
                </span>
            @else
                <span class="text-crt-navy font-extrabold bg-crt-cyan-light text-crt-navy px-2.5 py-0.5 rounded-md border border-crt-cyan/20">
                    Années Financières
