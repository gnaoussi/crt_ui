<div x-data x-effect="document.body.classList.toggle('overflow-hidden', $wire.isNewEmployeeModalOpen || $wire.isEditEmployeeModalOpen || $wire.isEditManagerModalOpen || $wire.isEditHoursModalOpen || $wire.isEditSiteModalOpen)" class="p-6 space-y-6 max-w-[1600px] mx-auto w-full">
    <!-- Floating Toast Notification (Template Style) -->
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4500)" x-transition:leave="transition ease-in duration-300 transform" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="fixed top-4 right-4 z-[110] flex items-center gap-3 px-4 py-3 rounded-xl shadow-xl bg-emerald-600 text-white animate-fade-in">
            <svg class="w-5 h-5 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="text-sm font-medium">{{ session('message') }}</span>
            <button @click="show = false" class="ml-2 text-white/80 hover:text-white p-0.5 cursor-pointer">
    ...
    @else
        ...
        <div class="space-y-6">
            <div>
                <button wire:click="backToList" class="text-xs font-bold text-slate-600 bg-white border border-slate-300 hover:bg-slate-50 px-3.5 py-2 rounded-xl transition"><span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-slate-500 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg> Retour à la liste des employés</span></button>
            </div>

            <div class="bg-gradient-to-r from-crt-cyan to-crt-navy rounded-2xl p-8 text-center text-white shadow-lg">
                <div class="w-20 h-20 rounded-full bg-white text-crt-navy flex items-center justify-center mx-auto mb-3 shadow-md border border-crt-cyan/30"><svg class="w-10 h-10 text-crt-navy" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>
                <h2 class="text-xl font-black">{{ $selectedEmployee->nom }} {{ $selectedEmployee->prenom }}</h2>
                <p class="text-xs text-crt-cyan-light font-mono mt-1">Matricule : {{ $selectedEmployee->matricule }} — {{ $selectedEmployee->role }}</p>
            </div>
            ...
            <div class="flex justify-between items-center bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                <div class="flex space-x-2 text-xs font-extrabold">
                    <button wire:click="setTab('information')" class="px-4 py-2.5 rounded-xl transition {{ $employeeActiveTab === 'information' ? 'bg-crt-navy text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100' }}"><span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-crt-cyan inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> Informations de l'employé</span></button>
                    <button wire:click="setTab('historiques')" class="px-4 py-2.5 rounded-xl transition {{ $employeeActiveTab === 'historiques' ? 'bg-crt-navy text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100' }}"><span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-crt-cyan inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> Historiques</span></button>
                </div>
            </div>
